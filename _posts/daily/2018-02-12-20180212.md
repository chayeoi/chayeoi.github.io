...
2. `/reviewInfo/$course/$uid/$review`에 저장된 데이터를 삭제한다.
3. `/courses/category/$course`에 저장된 `reviewCount`를 1만큼 감소시킨다.
4. `courses/category/$course`에 저장된 `ratingAvg` * `reviewCount`에 `myReviews/$uid/$course/$review`의 `rating`을 뺀 후, `reviewCount - 1`로 나눈 값을 `courses/category/$course`의 `ratingAvg`에 새로 저장한다.
